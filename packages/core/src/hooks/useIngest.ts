...
/** Where a phone POSTs captured bank SMS. */
export const smsIngestUrl = () => `${getSupabaseUrl()}/functions/v1/sms-ingest`
...
export function useReprocess() {
  ...
  return useMutation({
    mutationFn: async (messageId?: string): Promise<ReprocessSummary> => {
      const { data, error } = await getSupabase().functions.invoke('sms-reprocess', {
